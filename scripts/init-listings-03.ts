...
(async () => {
    ...
    const allDepartments = Array
        .from({ length: 94 }, (_, i) => i + 1).reverse();
    ...
    for (const energyClass of ENERGY_CLASSES) {
        for (const department of allDepartments) {
            const success = await createMoteurImmoJob({
                ...
                afterDate: threeMonthsAgo.toISOString().split('T')[0],
                energyClass,
            });
        ...
        console.log(`✅ Completed energy class ${energyClass} for months period\n`);
    }
...
function buildCreateMoteurImmoJobBody(endpointUrl: string, headers: Record<string, string>) {
    return async function createMoteurImmoJob({
        ...
        afterDate,
        energyClass,
    }: { departmentId: number, afterDate: string, energyClass: string }): Promise<boolean> {
        try {
            ...
            if (!response.ok) {
                ...
                console.error(`Failed to create job for department ${departmentId}, period ${afterDate}, energy class ${energyClass}: ${response.status} ${response.statusText} - ${errorText}`);
                return false;
            }

            await response.text();
            //console.log(`✅ Created listings job for department ${departmentId}, period ${afterDate}, energy class ${energyClass}`);
            return true;

        } catch (error) {
            console.error(`❌ Error creating job for department ${departmentId}, period ${afterDate}, energy class ${energyClass}:`, error);
            return false;
        }
    };
}