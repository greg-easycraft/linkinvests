export interface DpeRecord {
  numero_energyClass: string; // DPE certificate number
  ...
  etiquette_energyClass: string; // Energy efficiency rating (A-G)
  etiquette_ges: string; // GHG emissions rating (A-G)
  _geopoint: string; // Coordinates in format "lat,lon"
  date_etablissement_energyClass?: string; // DPE establishment date (YYYY-MM-DD)
  date_reception_energyClass?: string; // DPE reception date (YYYY-MM-DD)
