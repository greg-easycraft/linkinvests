...
export const FRENCH_DEPARTMENTS: DepartmentOption[] = [
  { id: 1, name: "Ain", label: "01 - Ain" },
  { id: 2, name: "Aisne", label: "02 - Aisne" },
  { id: 3, name: "Allier", label: "03 - Allier" },
  { id: 4, name: "Alpes-de-Haute-Provence", label: "04 - Alpes-de-Haute-Provence" },
  { id: 5, name: "Hautes-Alpes", label: "05 - Hautes-Alpes" },
  { id: 6, name: "Alpes-Maritimes", label: "06 - Alpes-Maritimes" },
  { id: 7, name: "Ardèche", label: "07 - Ardèche" },
  { id: 8, name: "Ardennes", label: "08 - Ardennes" },
  { id: 9, name: "Ariège", label: "09 - Ariège" },
  ...
  { id: 64, name: "Pyrénées-Atlantiques", label: "64 - Pyrénées-Atlantiques" },
  ...
  { id: 90, name: "Territoire de Belfort", label: "90 - Territoire de Belfort" },
...
export function searchDepartments(query: string): DepartmentOption[] {
  ...
  return FRENCH_DEPARTMENTS.filter(dept => {
    const matchesId = dept.id.toString().includes(normalizedQuery);
    const matchesName = dept.name.toLowerCase().includes(normalizedQuery);
    const matchesLabel = dept.label.toLowerCase().includes(normalizedQuery);
 ...
 * @returns Department option or undefined if not found
 */
export function getDepartmentById(id: number): DepartmentOption | undefined {
  return FRENCH_DEPARTMENTS.find(dept => dept.id === id);
}

/**
 * Get multiple departments by their IDs
 * @param ids - Array of department IDs
 * @returns Array of found departments
 */
export function getDepartmentsByIds(ids: number[]): DepartmentOption[] {
