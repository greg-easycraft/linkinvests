...
export class S3Service {
  ...
  async uploadImage(
    ...
    try {
      this.logger.log(`Uploading image to S3: ${key}`);
      ...
      this.logger.log(`Image uploaded successfully: ${s3Path}`);
  ...
  /**
   * Generate a unique S3 key for an auction image
   * @param departmentId - Department number
   * @param auctionId - Unique auction identifier (could be hash or ID)
   * @param imageType - Type of image (e.g., 'main', 'detail-1')
   * @returns S3 key in format: auctions/dept-{id}/{auctionId}/{imageType}.jpg
   */
  generateAuctionImageKey(
    departmentId: number,
    auctionId: string,
    imageType: string = 'main'
  ): string {
    return `auctions/dept-${departmentId}/${auctionId}/${imageType}.jpg`;
  }
}
