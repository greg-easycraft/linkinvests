...
export function AddressSearchPage(): React.ReactElement {
  ...
  const [selectedEnergyDiagnostic, setSelectedEnergyDiagnostic] =
    useState<EnergyDiagnostic | null>(null)
  ...
  const handleSubmit = async (data: AddressSearchInput) => {
    ...
    try {
      const searchResults = await searchAddresses(data)
      setResults(searchResults)
      setHasSearched(true)

      if (searchResults.length === 0) {
        setError(
          "Aucune adresse correspondante trouvée. Essayez d'ajuster vos critères de recherche.",
        )
      }
    } catch (err) {
      const errorMessage =
  ...
  const handleViewDetails = async (result: AddressSearchResult) => {
    ...
    try {
      const diagnostic = await getEnergyDiagnosticByExternalId(
      ...
      if (diagnostic) {
        ...
        setError(
          'Échec du chargement des informations détaillées pour cette adresse.',
        )
      }
    } catch (err) {
      const errorMessage =
        err instanceof Error ? err.message : 'Échec du chargement des détails'
      setError(errorMessage)
  ...
  return (
    <div className="container mx-auto py-8 space-y-8 max-w-6xl">
      ...
      {error && (
        <Alert
          variant="destructive"
          className="max-w-4xl bg-card shadow-sm mx-auto"
        >
      ...
      {hasSearched && results.length > 0 && (
        <Alert className="max-w-4xl mx-auto border-green-200 bg-green-50">
          ...
          <AlertDescription className="text-green-700">
            ...
            correspondante{results.length === 1 ? '' : 's'} trouvée
            {results.length === 1 ? '' : 's'} selon vos critères. Les résultats
            sont classés par niveau de confiance.
          </AlertDescription>
        </Alert>
      )}
      ...
      {isLoading && (
        <Alert className="max-w-4xl mx-auto border-blue-200 bg-blue-50">
          ...
          <AlertDescription className="text-blue-700">
            Analyse des dossiers de diagnostics énergétiques pour trouver les
      ...
      {/* Search Results */}
      {(hasSearched || results.length > 0) && (
        <SearchResults results={results} onViewDetails={handleViewDetails} />
      )}
          ...
          <div className="space-y-4">
            ...
            <div className="text-sm space-y-2">
              <p className="font-medium">
                Essayez ces conseils pour améliorer votre recherche :
              </p>
              <ul className="list-disc list-inside space-y-1 text-left max-w-md mx-auto">
                <li>
                  Retirez le filtre de classe énergétique DPE pour inclure plus
                  de propriétés
                </li>
